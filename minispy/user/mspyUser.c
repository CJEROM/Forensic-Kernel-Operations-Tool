...

{
    ...
    printf("    %ws\n", buffer);
    WriteAlertToDatabase("%ws", buffer);
}
...
--*/
{
    ...
    WriteAlertToDatabase("Connecting to filter's port...");
    ...
    if (IS_ERROR( hResult )) {
        ...
        WriteAlertToDatabase("Could not connect to filter: 0x%08x", hResult);
    ...
    if (context.ShutDown == NULL) {
        ...
        WriteAlertToDatabase("Could not create semaphore: %d", result);

        DisplayError( result );
        goto Main_Exit;
    }
    WriteAlertToDatabase("Semaphore created succesfully!");
    ...
    WriteAlertToDatabase("Creating logging thread...");
    ...
    if (!thread) {
        ...
        WriteAlertToDatabase("Could not create logging thread: %d", result);
        DisplayError( result );
        goto Main_Exit;
    }
    WriteAlertToDatabase("Logging thread created successfully!");
    ...
    while (inputChar = (CHAR)getchar()) {
        ...
        if (inputChar == '\n') {
            ...
            WriteAlertToDatabase("Entered minispy command terminal!");
...
Main_Cleanup:
    ...
    printf( "Cleaning up...\n" );
    WriteAlertToDatabase("Cleaning up threads...");
...
Main_Exit:
    ...
    WriteAlertToDatabase("Shutting down!");
    return 0;
}
        ...
        if (parm[0] == '/') {
            ...
            case 'A':
                ...
                if (SUCCEEDED( hResult )) {
                    ...
                    WriteAlertToDatabase("Instance name: %S", instanceName);

                } else {

                    printf( "\n    Could not attach to device: 0x%08x\n", hResult );
                    WriteAlertToDatabase("Could not attach to device : 0x % 08x", hResult);
                    DisplayError( hResult );
                    returnValue = SUCCESS;
                }
            ...
            case 'D':
                ...
                WriteAlertToDatabase("Detaching from %s", parm);
                bufferLength = MultiByteToWideChar( CP_ACP,
                                                    MB_ERR_INVALID_CHARS,
                                                    parm,
                                                    -1,
                ...
                if (IS_ERROR( hResult )) {
                    ...
                    WriteAlertToDatabase("Could not detach from device: 0x%08x", hResult);
                    DisplayError( hResult );
                    returnValue = SUCCESS;
                }
                break;
            ...
            default:
                ...
                //
                goto InterpretCommand_Usage;
                WriteAlertToDatabase("Invalid Command Entered into minispy terminal");
            }
